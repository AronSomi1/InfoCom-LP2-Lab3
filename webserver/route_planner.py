...
def route_planner():
    ...
    current_location = (
        float(redis_server.get("longitude")),
        float(redis_server.get("latitude")),
    )
    ...
    if from_location is None:
        ...
        subprocess.Popen(
            [
                ...
                "../pi/pi_controller.py",
                "--clong",
                str(current_location[0]),
                "--clat",
                str(current_location[1]),
                "--flong",
                str(from_location.longitude),
                "--flat",
                str(from_location.latitude),
                "--tlong",
                str(to_location.longitude),
                "--tlat",
                str(to_location.latitude),
            ]
        )
    return message
